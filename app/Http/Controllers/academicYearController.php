...
class academicYearController extends Controller
{
    ...
    public function create()
    {
        if (\Input::has('isDefault') AND \Input::get('isDefault') == 1) {
            \DB::table('academic_year')->update(['isDefault' => 0]);
            $isDefault = 1;
        } else {
            $isDefault = 0;
        }
        ...
        return $this->panelInit->apiOutput(true, $this->panelInit->language['addAcademicyear'], $this->panelInit->language['acYearAddSuc'], ["id" => $academicYear->id, "yearTitle" => \Input::get('yearTitle'), "isDefault" => $isDefault]);
    }
    ...
    function edit($id)
    {
        ...
        return $this->panelInit->apiOutput(true, $this->panelInit->language['editAcademicYears'], $this->panelInit->language['acYearModSuc'], ["id" => $academicYear->id, "yearTitle" => \Input::get('yearTitle'), "isDefault" => $academicYear->isDefault]);
    }
    ...
    function active($id)
    {
        ...
        $academicYear->save();
        \Session::put('selectAcYear', $id);

        return $this->panelInit->apiOutput(true, $this->panelInit->language['editAcademicYears'], $this->panelInit->language['acYearNowDef'], ["id" => $academicYear->id]);
    }
